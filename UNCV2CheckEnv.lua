...
local function test(name, aliases, callback, _function, index)
    running = running + 1
    local func = _function
    if index and _function then --[[
    quick fix for just in case
    if example thing like cache does not exist and script tries to index something in it like "invalidate" (cache.invalidate), errors and UNC test does not work
    ]]
        func = _function[index]
    end
    task.spawn(function()
        if name == "script" then
            local success, message = pcall(function() return script ~= nil end)
            if message then -- supposed to be message
